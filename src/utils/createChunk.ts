...
const createChunk = (compilation: Compilation, chunkAssets: Chunk['auxiliaryFiles'], tenantName: string, id: string, assets: string[], ext: string, createSource: CreateSource, targetExt = ext) => {
  const fileRegEx = new RegExp(`\\.${ext}$`, 'u'),
   files = Array.from(chunkAssets).filter((assetFile) => (!tenantName || assetFile.startsWith(tenantName)) && fileRegEx.test(assetFile));
